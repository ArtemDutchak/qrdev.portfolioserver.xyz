...
class Customer extends \Opencart\System\Engine\Model {
	public function addCustomer(array $data): int {
		...
		`is_activated` = '" . (isset($data['is_activated']) ? (bool)$data['is_activated'] : 0) . "',
		`safe` = '" . (isset($data['safe']) ? (bool)$data['safe'] : 0) . "',
		`date_added` = NOW()");

		$customer_id = $this->db->getLastId();
		
		if ((int)$data['tariff_id']) {
			
			$date_to = date('Y-m-d', strtotime($data['active_date_to']));
			$date_to = $date_to . ' 23:59:59';
			
			$date_activated = date('Y-m-d H:i:s', time());
			
			$this->db->query(
				"INSERT INTO
					`" . DB_PREFIX . "customer_tariff`
				SET
					`customer_id` = '" . (int)$customer_id . "',
					`tariff_id` = '" . (int)$data['tariff_id'] . "',
					`active_to` = '" . $this->db->escape($date_to) . "',
					`date_activated` = '" . $this->db->escape($date_activated) . "'
				");
		}
	...
	public function editCustomer(int $customer_id, array $data): void {
		...
		$exist_tariff_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_tariff` WHERE `customer_id` = '" . (int)$customer_id . "'");
		$exist_tariff = $exist_tariff_query->row;
		
		if ($exist_tariff && (int)$data['tariff_id'] && (int)$exist_tariff['tariff_id'] == (int)$data['tariff_id']) {
			$date_activated = $exist_tariff['date_activated'];
		}else{
			$date_activated = date('Y-m-d H:i:s', time());
		}
		
		$this->db->query("DELETE FROM `" . DB_PREFIX . "customer_tariff` WHERE `customer_id` = '" . (int)$customer_id . "'");
		if ((int)$data['tariff_id']) {
	...
	public function deleteCustomer(int $customer_id): void {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "address` WHERE `customer_id` = '" . (int)$customer_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "company` WHERE `customer_id` = '" . (int)$customer_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "customer_tariff` WHERE `customer_id` = '" . (int)$customer_id . "'");
	}
	...
	public function getCustomerCompanies(int $customer_id): array {
		$query = $this->db->query("SELECT
		*
