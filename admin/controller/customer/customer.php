...
class Customer extends \Opencart\System\Engine\Controller {
	...
	protected function getList(): string {
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = [
				...
				'is_activated'   => ($result['is_activated'] ? $this->language->get('text_is_activated') : $this->language->get('text_is_not_activated')),
	...
	public function form(): void {
		...
		$data['tariffs'] = $this->model_catalog_tariff->getTariffs();
		$data['customer_id'] = 0;
		$data['current_tariff_id'] = 0;
		$data['date_active_to'] = date('Y-m-d', strtotime("+1 months", time()));
		...
		if (!empty($customer_info)) {
			$data['is_activated'] = $customer_info['is_activated'];
		} else {
			$data['is_activated'] = 0;
		}
	...
	public function save(): void {
		...
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {
				...
				// if ((Helper\Utf8\strlen($value['lastname']) < 1) || (Helper\Utf8\strlen($value['lastname']) > 32)) {
				// 	$json['error']['address_' . $key . '_lastname'] = $this->language->get('error_lastname');
				// }

				if ((Helper\Utf8\strlen($value['address_1']) < 3) || (Helper\Utf8\strlen($value['address_1']) > 128)) {
					$json['error']['address_' . $key . '_address_1'] = $this->language->get('error_address_1');
				}

				if ((Helper\Utf8\strlen($value['city']) < 2) || (Helper\Utf8\strlen($value['city']) > 128)) {
