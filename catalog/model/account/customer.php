...
class Customer extends \Opencart\System\Engine\Model {
	public function addCustomer(array $data): int {
		...
		`is_activated` = '" . (isset($data['is_activated']) ? (int)$data['is_activated'] : 0) . "',
		`customer_code` = '" . md5($this->db->escape((string)$data['email'])) . "',
		`date_added` = NOW()");
	...
	public function getCustomerByActivationCode(string $code): array {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer` WHERE `customer_code` = '" . $this->db->escape($code) . "' AND `customer_code` != ''");

		return $query->row;
	}

	public function setActivation(int $customer_id, bool $status): void {
		$query = $this->db->query("UPDATE `" . DB_PREFIX . "customer` SET `is_activated` = '" . $status . "' WHERE `customer_id` = '" . $customer_id . "'");
	}

	public function setFreeAble(int $customer_id, bool $status): void {
		$query = $this->db->query("UPDATE `" . DB_PREFIX . "customer` SET `free_able` = '" . $status . "' WHERE `customer_id` = '" . $customer_id . "'");
	}

	public function getFreeAble(int $customer_id): bool {
		$query = $this->db->query("SELECT free_able FROM `" . DB_PREFIX . "customer` WHERE `customer_id` = '" . $customer_id . "'");
		
		if ($query->row) {
			return (bool)$query->row['free_able'];
		}
