...
class Login extends \Opencart\System\Engine\Controller {
	...
	public function login(): void {
		...
		if ($this->customer->isLogged()) {
			$json['redirect'] = $this->url->link('account/reviews', 'customer_token=' . $this->session->data['customer_token'], true);
		}
		...
		if (!$json['errors']) {
			...
			$email = trim($this->request->post['email_log_in']);
			$password = trim($this->request->post['password_log_in']);
			...
			} elseif (!$this->customer->login($email, html_entity_decode($password, ENT_QUOTES, 'UTF-8'))) {
