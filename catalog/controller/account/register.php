...
class Register extends \Opencart\System\Engine\Controller {
	...
	public function register(): void {
		...
		// format data
		$this->request->post['firstname']    = $firstname;
		$this->request->post['email']        = $email;
		$this->request->post['telephone']    = $telephone;
		$this->request->post['company_name'] = $company_name;
		...
		if (!$json['errors']) {
			...
			if ((Helper\Utf8\strlen($firstname) < 1) || (Helper\Utf8\strlen($firstname) > 32)) {
				$json['error']['firstname'] = $this->language->get('error_firstname');
				$json['errors'][] = array(
					'field_name' => 'firstname',
					'text' => $this->language->get('error_firstname'),
				);
			}
			...
			if ($this->config->get('config_telephone_required') && (Helper\Utf8\strlen($telephone) != 12)) {
			...
			if ((Helper\Utf8\strlen(html_entity_decode($company_name, ENT_QUOTES, 'UTF-8')) < 1) || (Helper\Utf8\strlen(html_entity_decode($company_name, ENT_QUOTES, 'UTF-8')) > 40)) {
			...
			if ((Helper\Utf8\strlen(html_entity_decode($password, ENT_QUOTES, 'UTF-8')) < 4) || (Helper\Utf8\strlen(html_entity_decode($password, ENT_QUOTES, 'UTF-8')) > 40)) {
				$json['error']['password'] = $this->language->get('error_password');
				$json['errors'][] = array(
					'field_name' => 'password',
					'text' => $this->language->get('error_password'),
				);
			}
		...
		if (!$json['errors']) {
			...
			$json['redirect'] = $this->url->link('account/success|register');
		}
