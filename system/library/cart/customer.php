...
class Customer {
	...
	public function getCompanyInfo(int $company_id): array {
		...
		*
		...
		AND `customer_id` = '" . (int)$this->customer_id . "'
		ORDER BY date_added DESC
		");
	...
	public function getCurrentTariffId(): int {
		$query = $this->db->query("SELECT
		tariff_id
		...
		if ($query->row) {
			return (int)$query->row['tariff_id'];
		}

		return 0;
	}

	public function getCurrentTariffInfo(): array {
		$query = $this->db->query("SELECT
		ct.tariff_id as tariff_id,
		ct.active_to as active_to,
		ct.date_activated as date_activated,
		t.companies as companies,
		t.status as status,
		td.name as name
		FROM `" . DB_PREFIX . "customer_tariff` ct
		LEFT JOIN `" . DB_PREFIX . "tariff` t ON (ct.tariff_id = t.tariff_id)
		LEFT JOIN `" . DB_PREFIX . "tariff_description` td ON (ct.tariff_id = td.tariff_id)
		WHERE
		`customer_id` = '" . (int)$this->customer_id . "' AND
